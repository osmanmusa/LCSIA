...
def shrink_one_bit_F(y, p, pvar, sigmaw2, M, theta, code_symbol):
    """
    threshold function from my thesis, page 70

    so far I do not use any scaling
    """
    tf_floattype = p.dtype
    y = tf.dtypes.cast(y, dtype=tf_floattype)
    sigmaw2 = tf.dtypes.cast(sigmaw2, dtype=tf_floattype)

    OneOverM = tf.constant(float(1) / M, dtype=tf_floattype)

    p2 = tf.math.square(p)
    tfd = tfp.distributions
    z_dist = tfd.Normal(loc=p, scale=tf.sqrt(pvar))

    p0 = z_dist.cdf(0)
    p1 = 1-p0

    m0 = p*p0 - pvar*z_dist.prob(0)
    m1 = p*p1 + pvar*z_dist.prob(0)

    s0 = p0*(p2 + pvar) - pvar*p*z_dist.prob(0)
    s1 = p1*(p2 + pvar) + pvar*p*z_dist.prob(0)

    noise_dist = tfd.Normal(loc=tf.constant(0., dtype=tf_floattype), scale=tf.sqrt(sigmaw2))

    f_y = p0*noise_dist.prob(y+code_symbol) + p1*noise_dist.prob(y-code_symbol)
    E = (m0*noise_dist.prob(y+code_symbol) + m1*noise_dist.prob(y-code_symbol))/f_y
    P = (s0*noise_dist.prob(y+code_symbol) + s1*noise_dist.prob(y-code_symbol))/f_y
    var = P - tf.math.square(E)

    F1 = (E - p)/pvar
    F2 = (pvar - var)/tf.math.square(pvar)
...
def get_shrinkage_function(name, pnz = 0.1):
	"retrieve a shrinkage function and some (probably awful) default parameter values"
    # pnz - Probability of a nonzero.
	try:
		return {
            'soft': (shrink_soft_threshold, (1., 1.)),
            'bg':(shrink_bgest, (1,math.log(1/pnz-1)) ),
            'onebitF': (shrink_one_bit_F, (1., 1.)),
            'nn': (shrink_soft_threshold, (2., 1.)),
		}[name]
	except KeyError:
		raise ValueError('unrecognized shrink function %s' % name)
		sys.exit(1)
